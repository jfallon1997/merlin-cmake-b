...
*   \class ACG
*
*   This is an extension of the older implementation of Algorithm M which I
*   previously supplied. The main difference between this version and the old
*   code are:
*
...
*   each being unique in that sequence (it has only 32 bits of state). It
...
*       8-bit counter (that has a period length of only 256).  The advantage of
*       an LCA is that it is perfectly uniform when run for the entire period
*       length and very uniform for smaller sequences too, if the parameter
*       are chosen carefully).
*
*   ACGs have extremely long period lengths and provide good independence. 
*   Unfortunately, uniformity isn't not too great. Furthermore, I didn't find
*   any theoretically analysis of ACGs that addresses uniformity.
*
*   The RNG given below will return numbers generated by an LCA that are
*   permuted under control of a ACG. 2 permutations take place: the 4 bytes of
*   one LCG generated number are subjected to one of 16 permutations selected
*   by 4 bits of the ACG. The permutation a such that byte of the result may
*   come from each byte of the LCG number. This effectively destroys the
*   structure within a word. Finally, the sequence of such numbers is permuted
*   within a range of 256 numbers. This greatly improves independence.
*
*
*  Algorithm M as describes in Knuths "Art of Computer Programming", Vol 2. 1969
...
*  i=1..512) results in no obvious structure (A(i) ~ const).
*
...
/**
*   Part of the table on page 28 of Knuth, vol II. This allows us to adjust the
*   size of the table at the expense of shorter sequences.
*/
